...
<div class="container pt-2">
    <div class="row">
        <div class="col-md-4 mr-auto">
            <form action="${pageContext.request.contextPath}/controller?command=tariffsPanel" method="post">
                    ...
                    <div class="col-md align-self-center">
                        <c:choose>
                            <c:when test="${tariff.status=='ACTIVE'}">
                                <p class="card-text text-success bold"><fmt:message key="lang.status"/> <c:out
                                        value="${tariff.status}"/></p>
                            </c:when>
                            <c:when test="${tariff.status=='ARCHIVE'}">
                                <p class="card-text text-warning bold"><fmt:message key="lang.status"/> <c:out
                                        value="${tariff.status}"/></p>
                            </c:when>
                            <c:when test="${tariff.status=='DEACTIVATED'}">
                                <p class="card-text text-danger bold"><fmt:message key="lang.status"/> <c:out
                                        value="${tariff.status}"/></p>
                            </c:when>
                        </c:choose>
